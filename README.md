...
Кроки 1-4 в форматі відоса - https://youtu.be/DZfO1HATD4E
...
Спочатку треба украсти проект собі на Github.
1. Зайти на https://github.com, авторизуватись
2. Зайти на https://github.com/v3il/TestProject
3. Натиснуть кнопку Fork - https://i.imgur.com/acRTqtX.png
4. Перейти у свій склонований репозиторій і скопіювати лінку на нього - https://i.imgur.com/cf5icLm.png
...
4. Виконай команду `git clone ЛІНКА_НА_ТВІЙ_РЕПО`. Юзай `Ctrl/Shift + Insert` замість `Ctrl + C/V` або `ПКМ -> Copy/Paste`. 
...
`dependencies` - список бібліотек, які використовуються в проекті (відсутнє в цьому проекті, бо я писав все з нуля). 
...
Наприклад, `Rollup` використовується для упаковки всіх JS файлів в один для удобного його підключення в HTML файл, `eslint` використовується
для лінтінгу скриптів (щоб стиль написання всіх JS файлів був однаковий і підпорядковувався певним правилам), а jest - для написання і запуску тестів.
...
### 5. Перевірка коду
Я написав тести (код, який провіряє код, угу) на основні модулі, так шо коли закінчиш роботу над якимось завданням, можна
виконати команду `npm run test` і перевірити свою реалізвацію. Треба, щоб всюди були галочки - https://i.imgur.com/RD6zUWz.png :D

Всьо, можна писать код 🎉, glhf
...
в діапазоні [1975-2000]. Додатково встанови 0 годин, хвилин, секунд і мілісекунд.
...
Один злобний чєл (я) перехватив твої дані про юзерів і трошки їх поламав. Потрібно реалізувати методи класу і відновити 
дані користувачів.

**Завдання:**

1. Реалізувати метод `parseUsersJson`. Метод приймає 1 параметр - `usersJSON` - масив об'єктів з 
поламаними даними про користувачів, закодований в форматі **JSON (JSON-рядок)** і має повернути розпарсений масив користувачів. 
2. Реалізувати метод `fixUser`. Метод приймає 1 параметр - `brokenUser` - об'єкт з поламаними даними про користувача, який містить такі поля:
...
Кожне поле треба буде "очистити", якось діставши оригінальне значення поля. Метод має повернути об'єкт з полями `id, name, lastName, avatarId, birthDate` з 
...
2. Реалізувати у класі `User` метод `getData`. Метод має повертати об'єкт з такими полями:
...
у HTML розмітку. Приклад - шаблон `<div id="id1" class="{{ cls }}">{{ text }}</div>` (пробіли в {{ ... }} **обов'язкові!**) і об'єкт з даними `{ cls: 'test', text: 'TEXT' }`
...
Для кожного очищеного значення треба повернути по 2 об'єкта, кожен з яких мають такі поля:
...
### 7. Реалізація фільтрації і сортування користувачів
Потрібний файл знаходиться за шляхом `src/services/UsersService.js`.

**Завдання:**
1. Над таблицею користувачів є поле, за допомогою якого можна шукати користувачів за ім'ям і прізвищем. В указаному файлі 
необхідно реалізувати метод `filterUsers`. Параметром приходить масив користувачів, які потрібно відфільтрувати, а також 
дані про фільтри. Параметр `filters` являє собою об'єкт, в якого є поле `query`. В ньому лежить пошуковий запит, який ввели в поле.
Необхідно відібрати користувачів, у яких в полі `fullName` є підрядок `query` (регістр неважливий) і повернути масив таких користувачів.
2. Над таблицею користувачів є селект, за допомогою якого можна сортувати користувачів за певними критеріями і в певному порядку. 
В указаному файлі необхідно реалізувати метод `sortUsers`. Параметром приходить масив користувачів, які потрібно відсортувати, а також
дані про сортування: по чому сортуємо і порядок сортування (від меншого до більшого чи навпаки). 
Параметр `sorts` являє собою об'єкт, в якого є поле `sortBy`. В ньому лежить рядок, який вказує на те, по чому будемо сортувати.
Може набувати одного з трьох значень: `full_name` - сортуємо по полю `fullName`, `posts` - сортуємо по кількості постів і `birth_date` - 
сортуємо по даті народження. Також у об'єкті `sorts` і поле order, яке вказує на порядок сортування і може набувати двох 
значень - `asc` (від меншого до більшого) i `desc` (від більшого до меншого). \
Метод повинен повернути відсортований по вказаному критерію і у вказаному порядку масив `users`.

**Поради до п. 2:**
1. Сортування рядків можна робить за допомогою https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/String/localeCompare
2. Перед сортуваннями порішай з порядком сортування (параметр `а` будеш порівнювать з `b` чи навпаки)
