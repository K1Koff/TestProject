...
У нас є один скрипт, який називається `dev` і потрібен для запуску проекту. Він запускає бібліотеку `Rollup`, яка збирає всі скрипти 
...
Далі просто відкрий `build/index.html` через VSCode в браузері, всі зміни в файлах будуть автоматом показуватись в браузері (інколи
